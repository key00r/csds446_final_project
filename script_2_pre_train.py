...
if 1:
    ...
    if dataset_name == 'pubmed':
        ...
        lr = 0.001
        k = 7
        max_epoch = 500 # ---- do an early stop when necessary ----
    elif dataset_name == 'citeseer':
        k = 5
        lr = 0.001
        max_epoch = 2000 # it takes a long epochs to converge, probably more than 2000
    ...
    torch.save(method_obj, './result/GraphBert/' + dataset_name + '_graphbert_pretrained.pth')
...
#qiyong---- Pre-Training Task #2: Graph Bert Network Structure Recovery (Cora, Citeseer, and Pubmed) ----
if 0:
    #---- hyper-parameters ----
    if dataset_name == 'pubmed':
        lr = 0.001
...
if 0:  # ← 启用 Task 2，确保这一行是 if 1
    ...
    print('************ Start ************')
    print('GraphBert, dataset:', dataset_name, ', Pre-training, Graph Structure Recovery.')
    ...
    # ✅ 保存结构恢复模型为 pth 文件（完整模型）
    torch.save(method_obj, './result/GraphBert/' + dataset_name + '_graphbert_structure_recovery.pth')
