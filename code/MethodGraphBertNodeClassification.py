...
class MethodGraphBertNodeClassification(BertPreTrainedModel):
    ...
    def train_model(self, max_epoch):
        ...
        for epoch in range(max_epoch):
            ...
            loss_train = loss_train if torch.is_tensor(loss_train) else torch.tensor(loss_train)
            acc_train = acc_train if torch.is_tensor(acc_train) else torch.tensor(acc_train)
            loss_val = loss_val if torch.is_tensor(loss_val) else torch.tensor(loss_val)
            acc_val = acc_val if torch.is_tensor(acc_val) else torch.tensor(acc_val)

            loss_test = loss_test if torch.is_tensor(loss_test) else torch.tensor(loss_test)
            acc_test = acc_test if torch.is_tensor(acc_test) else torch.tensor(acc_test)
